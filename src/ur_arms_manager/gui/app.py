...
def _serialize_robot_card(
    ...
    home_program = str(robot.home_program or "").strip()
    return {
        "name": robot.name,
        "host": robot.host,
        ...
        "home_program": home_program or "None configured",
        ...
        "can_move_home": bool(home_program),
        ...
        "home_disabled_reason": (
            "Configure home_program in config/robots.yaml before using Move Home."
            if robot.enabled and not home_program
            else (
                "Robot is disabled in config. Enable it before sending commands."
                if not robot.enabled
                else None
